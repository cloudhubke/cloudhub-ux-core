...
export default class ImageGallery extends React.Component {
  ...
  static defaultProps = {
    ...
    captionElement: ({ item }) => <label>{item ? item.caption : ''}</label>,
  };
  ...
  renderCaption() {
    ...
    const item = Array.isArray(items) ? items[currentIndex] : {};
    return this.props.captionElement({ item, items, currentIndex, maxWidth });
  }
      ...
      <div ref={this.imageGallery} className={igClass} aria-live="polite">
        <div className={igContentClass}>
          {(thumbnailPosition === 'bottom' || thumbnailPosition === 'right') &&
            slideWrapper}
          {this.props.showCaption && this.renderCaption()}
          {showThumbnails && (
            <div
